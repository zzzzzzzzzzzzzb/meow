...
local shimmyForwardDistance = 1.2 -- studs, just touching their back (not inside)
local shimmyBackwardDistance = 2.5 -- studs, bigger backward step during shimmy
...
local function followAndShimmy(targetTorso, myHRP)
	...
	while shimmyActive do
		...
		local backDir = targetTorso.CFrame.LookVector

		local torsoPos = targetTorso.Position

		-- Move to just outside their back (forward)
		local forwardPos = torsoPos - backDir * shimmyForwardDistance
		...
		local forwardCFrame = CFrame.new(forwardPos, forwardPos + lookDir)
		myHRP.CFrame = forwardCFrame
		task.wait(shimmySpeed)

		-- Move backward a bit more than forward distance
		local backPos = torsoPos - backDir * shimmyBackwardDistance
